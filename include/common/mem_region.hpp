...
struct mem_region_t {
    const uint          version;            // Checkpoint version.
    const uint          uid;                // Unique memory region identifier.
    const char*         ptr;                // Pointer of this memory region (can be either on GPU/CPU)
    ...
    mem_region_t(const uint version_, const uint uid_, const char* const ptr_, 
