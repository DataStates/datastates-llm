...
volatile static uint internal_uid_counter = 1;
struct mem_region_t {
    ...
    mem_region_t(uint version_, uint uid_, char* ptr_, 
        ...
        version(version_), uid(uid_), ptr(ptr_), size(size_), aligned_size(0), file_start_offset(file_start_offset_), path(path_), curr_tier_type(tier) { 
            internal_uid = internal_uid_counter++;
        };
    mem_region_t(const mem_region_t* other, TIER_TYPES next_tier): version(other->version), uid(other->uid), ptr(nullptr), size(other->size), aligned_size(other->aligned_size), file_start_offset(other->file_start_offset), path(other->path), curr_tier_type(next_tier), internal_uid(other->internal_uid) {};
    mem_region_t(const std::shared_ptr<mem_region_t> other, TIER_TYPES next_tier): version(other->version), uid(other->uid), ptr(nullptr), size(other->size), aligned_size(other->aligned_size), file_start_offset(other->file_start_offset), path(other->path), curr_tier_type(next_tier), internal_uid(other->internal_uid) {};
