...
public:
    ...
    virtual void restore_region(std::shared_ptr<mem_region_t> m) = 0;
...
static std::atomic<bool> is_core_engine_active {true};
static std::shared_ptr<core_t> core_engine_instance {nullptr};
core_t* create_core_engine(size_t host_cache_size, int gpu_id, int rank = -1, bool use_io_uring = false, size_t fs_block_alignment = FS_BLOCK_SIZE_ALIGNMENT);
} // namespace datastates
