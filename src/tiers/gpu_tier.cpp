...
gpu_tier_t::gpu_tier_t(int gpu_id, unsigned int num_threads, size_t total_size, int rank): 
    base_tier_t(GPU_TIER, gpu_id, num_threads, total_size, rank) {
    ...
    mem_pool = std::make_shared<mem_pool_t>(start_ptr_, total_size, gpu_id, rank, GPU_TIER);
